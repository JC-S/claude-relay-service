        ...
        <div class="modal-scroll-content custom-scrollbar flex-1 overflow-y-auto">
          ...
          <div v-if="stats && hasLimits" class="mb-6">
            ...
            <div class="space-y-3 rounded-lg bg-gray-50 p-4 dark:bg-gray-700/50">
              ...
              <div v-if="Number(apiKey.weeklyFableCostLimit) > 0" class="space-y-1.5">
                <LimitProgressBar
                  :current="Number(weeklyFableCost) || 0"
                  label="Fable 周费用限制"
                  :limit="Number(apiKey.weeklyFableCostLimit) || 0"
                  :show-shine="true"
                  type="fable"
                />
                <div class="text-right text-xs text-gray-500 dark:text-gray-400">
                  已使用 {{ Math.min(fableUsagePercentage, 100).toFixed(1) }}%
                </div>
              </div>
...
const hasLimits = computed(() => {
  return (
    ...
    Number(props.apiKey.weeklyFableCostLimit) > 0 ||
...
const fableUsagePercentage = computed(() => {
  if (!weeklyFableCostLimit.value || weeklyFableCostLimit.value === 0) return 0
  return (weeklyFableCost.value / weeklyFableCostLimit.value) * 100
})
