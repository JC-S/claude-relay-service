...
支持 Claude (官方/Console)、Gemini、OpenAI Responses、Grok、AWS Bedrock、Azure OpenAI、Droid、CCR 等账户类型。
...
- API Key 认证字段仍用 SHA-256 哈希存储；自支持 reveal 起，新建/重新生成的 key 额外存一份 AES 加密的明文副本 `encryptedApiKey`（复用 commonHelper 加密），仅经鉴权后的专用 reveal 端点解密返回，列表/详情/统计/日志/脱敏导出均不得返回或记录明文；功能上线前的旧 key 无此副本、不可显示明文。单纯 Redis dump 不能还原明文，但 dump + ENCRYPTION_KEY 可以，风险模型与 OAuth token 类似
- 每个请求必须经过完整认证链（API Key → 权限 → 客户端限制 → 模型黑名单）
- 客户端断开时必须通过 AbortController 清理资源和并发计数。唯一例外是 Grok relay：为获取终态
  usage，可在可重置 idle timeout 和原请求 hard deadline 双重上限内继续 drain；任一上限到达后必须
  Abort 并释放账户槽
...
- **流式响应**: SSE 传输，实时捕获 usage；除上述有界 Grok drain 例外外，客户端断开时立即通过
